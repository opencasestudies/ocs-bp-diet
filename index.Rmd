...
With the exception of trans fatty acids, the consumption of all of these dietary factors seems to decrease with age.  Let's add gender to this plot as well.  We will also use the `viridis` package to change the colors.
...
We see similar patterns for males and females.  Now let's take the same look at the foods that were under-consumed:
...
It's hard to see the patterns across age in this plot!  Let's change by putting the dietary factors on the x-axis (instead of sex) and facet by sex (instead of dietary factor).
...
We could also display the data differently by putting age group on the x axis and letting color designate sex.  This allows us to more clearly see the pattern across age and clearly shows which dietary factors have differences by sex.
...
Finally, let's look on the really problematic foods -- those with relative percent above 100% or below 25%.
...
We see that for the foods that are over consumed (processed meat, red meat, sodium, and sugar-sweetened beverages), consumption appears to decrease with age.  For the two foods that are under consumed, consumption of nuts and seeds seems to rise and then fall with age, whereas consumption of polyunsaturated fatty acids is consistently very low across all age groups.
...
Before we do this let's change the labels of the foods by adding new line breaks so that they will fit more easily on our graphs.
