...
Lancet journal evaluated global dietary trends and the relationship of dietary factors with mortality and fertility.
...
Notice that it doesn't report rows being a different order becuase it first tells what rows have unique values or rows with a value that has a different number of frequency.
...
In order to make [inferences](https://www.britannica.com/science/inference-statistics) about these comparisons, it is helpful to perform statistical tests. These tests can help us to determine the strength of the association between sex, age group, and country identity with relative consumption of the dietary factors of interest. One way to look at the strength of association is to use a statistical method called linear regression.

You may have already learned that one can compare two groups using a $t$-test. For more information on the $t$-test see this [case study](https://opencasestudies.github.io/ocs-bp-rural-and-urban-obesity/){target="_blank"}.
...
It turns out that both the $t$-test and ANOVA are equivalent to specialized types of [regression](https://lindeloev.github.io/tests-as-linear/){target="_blank"}. We will use each of these tests to investigate patterns of consumption for dietary factors that contribute to health risk and we will look at how we can obtain equivalent results with regression.
...
We will start by considering **simple linear regression**, where we have one predictor variable and one outcome variable. Using the ordinary least squares method we can identify a line that best fits the data by minimizing the sum of the squared vertical distances between each point and the line. 
...
Now $\beta_{1}$, called Beta one, is our slope notation and $\beta_{0}$, Beta zero (also called Beta naught), is our intercept notation.
...
This concept has been extended to allow for comparisons of **different types of outcomes (the y's)**, to involve **various numbers of covariates (independent variables - the x's)**, and to allow for **different shapes of lines**. Of course in these more complex situations, we can't visualize these relationships easily with a line on a two-dimensional page, but the mathematical concept remains in some sense the same.
...
We have both continuous outcomes of raw consumption rates or percentages of consumption relative to the optimal guideline amounts as well as binary outcomes of "yes" or "no" about whether a population achieved the optimal amount of consumption.
...
1) Normality of the data for both groups (this is not as much of an issue if the number of observations is relatively large, i.e., total n > 30 - which is indeed the case for us!)
2) Equal variance between the two groups (make sure you do the correct test if the data are not normal)
...
We can evaluate if our data are [normally distributed](https://www.physiology.org/doi/full/10.1152/advan.00064.2017) by plotting the distribution and by creating [Q-Q plots](http://onlinestatbook.com/2/advanced_graphs/q-q_plots.html){target="_blank"}. 

<u>If our data are not normally distributed, we can consider these options:</u>
...
It's also important that if there are multiple predictor variables, that these are not correlated.
...
1) Normality of the data for both groups (this is not as much of an issue if the number of observations is relatively large, i.e., total n > 30. We can evaluate this assumption by plotting the distribution and by creating [Q-Q plots]([Q-Q Plots](http://onlinestatbook.com/2/advanced_graphs/q-q_plots.html){target="_blank"}))
2) Equal variance between the two groups (make sure you do the correct test to assess this if the data are not normal)
...
2) Mood's test using [`mood.test()`](https://files.eric.ed.gov/fulltext/ED065559.pdf) (use if the data are not normally distributed)  
...
1) Normality of the data for all tested groups (less of an issue if the number of observations is relatively large, i.e., total n > 30) 
2) Equal variance between the groups - aka the [Homogeneity of Variances assumption](https://uc-r.github.io/assumptions_homogeneity){target="_blank"} (make sure you do the correct test if the data are not normal) 
3) Balanced sample sizes of the groups 
4) Independent observations (or independent paired observations) 
