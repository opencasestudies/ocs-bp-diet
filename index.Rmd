...
all_equal(X, Y, ignore_row_order = FALSE)
```
`setequal()` will report `TRUE` or `FALSE` to indicate a difference in rows if they have the same column names. 
...
If we rename the column name in Y, the function will be able to indicate a difference. 
