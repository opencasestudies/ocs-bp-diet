...
Overall, though, we can conclude from these tests that we have enough evidence to reject the null hypothesis that there is no difference between the mean consumption of males and females ( or that `sex` has no association or influence on red meat consumption.) **Therefore, it appears that men consume significantly more red meat than females globally.** 

Let's choose another dietary factor to compare between males a females -- relative percent consumption of fruit.  Again, we'll consider the log-transformed version of this variable, since it appears to be more normally distributed.
```{r}
diet_and_guidelines %>%
  filter(food == "fruits") %>%
  ggplot(aes(x = log10(Relative_Percent))) +
...
Let's compared the paired $t$-test and the mixed effects model again:
```{r}
# paired t-test
...
Notice here that the mean of the differences value is indeed the same as the coefficient and the $t$-values and the $p$-values match. The $\beta_{1}$ coefficient value (slope) matches the mean difference calculated in the paired $t$-test. The intercept coefficient $\beta_{0}$ value of 1.56 (when sex is zero or female) also matches the mean value for the female group when calculated separately. Ideally you would want to check the residual plots of the model, but we will skip this for the sake of space. 
