...
# Matrix is bundled with the rendering image. Install the compatible archived
# release only when it is genuinely absent; rebuilding it on every preview can
# exceed the preview action's render time limit.
...
Also, while [gender](https://www.thetrevorproject.org/resources/article/understanding-gender-identities-and-pronouns/){target="_blank"} and [sex](https://www.who.int/genomics/gender/en/index1.html){target="_blank"} are not actually binary, the data used in this analysis only contains information for groups of individuals described as male or female. 
...
all_equal(X, Y, ignore_row_order = FALSE)
```
The `setequal()` function allows us to check if there's a difference between 2 tibbles. The `setequal()` function requires that column names match in order for it to check if there is a difference. 

```{r, eval=FALSE}

setequal(X, Y) # This will not work 
```

If we rename the column name in Y, the function will be able to indicate a difference. `setequal()` will report `TRUE` or `FALSE` to indicate a difference in rows if they have the same column names. 

```{r}
Y2 <- Y %>% rename(colname1 = colname2)
setequal(X, Y2)
```
...
What are the countries that have such high consumption rates? It would be nice to be able to know what countries have the largest data points. One way to do this is using a package called `ggiraph`. This package is really helpful for creating interactive graphs. 
...
Looks like the males in Austria and Argentina have the highest consumption.
...
Nice! Now let's create an interactive graph. 
...
[$t$-tests and ANOVA are equivalent to regression](https://scientificallysound.org/2017/06/08/$t$-test-as-linear-models-r/){target="_blank"} also see [here](https://medium.com/data-science/everything-is-just-a-regression-5a3bf22c459c){target="_blank"} and [here](https://lindeloev.github.io/tests-as-linear/){target="_blank"} about how many commonly known statistical tests are specialized forms of regression  
...
[Fligner-Killeen](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/fligner.test.html){target="_blank"}  
[Levene's test](https://www.itl.nist.gov/div898/handbook/eda/section3/eda35a.htm){target="_blank"}  
 

<u>Other helpful links:</u>

[Long and Wide Data Formats](https://stefvanbuuren.name/fimd/sec-longandwide.html){target="_blank"}    
...
[Multiple Testing](https://web.archive.org/web/20250311064147/https://www.gs.washington.edu/academics/courses/akey/56008/lecture/lecture10.pdf){target="_blank"}    
...
assisting in framing the major direction of the case study, [Ashkan Afshin](https://globalhealth.washington.edu/faculty/ashkan-afshin) and [Erin Mullany](http://www.healthdata.org/about/erin-mullany) for giving us access to the data, as well as Padmashri Saravanan and Allissa Dillman from [BioData Sage](https://www.biodatasage.com/) for reviewing and improving the case study.
