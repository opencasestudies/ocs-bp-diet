...
Wright, Carrie, and Ontiveros, Michael and Jager, Leah and Taub, Margaret and Hicks, Stephanie. (2020). https://www.opencasestudies.org/ocs-bp-diet/. Mental Health of American Youth (Version v1.0.0).
...
# we are working with the GBD about this... hope to fix this issue and then we will not need to include a note about this:
# For instructors, there may be something wrong with the data regarding trans fatty acids - as the data for Malaysia in the age separated document appears to be different from that in the age collapsed data:

agecollapsed <- diet_and_guidelines %>% filter(food == "trans fatty acids")%>% select(mean, location_name)
notagecollapsed <- all_age_diet_and_guidelines %>% filter(food == "trans fatty acids")%>% select(mean, location_name)
range(agecollapsed$mean)
range(notagecollapsed$mean)
notagecollapsed %>% arrange(desc(mean))
agecollapsed %>% arrange(desc(mean))
#hmmm the lowest value for malaysia is 106...
notagecollapsed%>% filter(location_name == "Malaysia") %>% arrange(desc(-mean))
# but the collapsed value is much much lower...
agecollapsed%>% filter(location_name == "Malaysia")

# this also looks problematic:
#not age collapsed:
sep_age_diet_data %>% filter(food == "sodium", location_name== "Malaysia") %>%summarise(min = min(mean))
#age collapsed:
diet_and_guidelines %>%filter(food == "sodium", location_name== "Malaysia") %>%summarise(min = min(mean))


sep_age_diet_data %>% filter(location_name == "Malaysia", food == "trans fatty acids") %>% pull(lower)
diet_and_guidelines %>% filter(location_name == "Malaysia", food == "trans fatty acids") %>% pull(lower)

### confirming with raw data:

diet_data <- readr::read_csv(here("docs", 
                       "dietary_risk_exposure_all_ages_2017.csv"))
sep_age_diet_data <- read_csv(here("docs", 
                       "dietary_risk_exposure_sep_ages_2017.csv"))
sep_age_diet_data %>% filter(location_name == "Malaysia", rei_name == "Diet high in trans fatty acids") %>% pull(lower)
diet_data %>% filter(location_name == "Malaysia", rei_name == "Diet high in trans fatty acids") %>% pull(lower)
sep_age_diet_data %>% filter(location_name == "Malaysia", rei_name == "Diet high in sodium") %>% pull(lower)
diet_data %>% filter(location_name == "Malaysia", rei_name == "Diet high in sodium") %>% pull(lower)
sep_age_diet_data %>% filter(location_name == "Malaysia", rei_name == "Diet low in polyunsaturated fatty acids") %>% pull(lower)
diet_data %>% filter(location_name == "Malaysia", rei_name == "Diet low in polyunsaturated fatty acids") %>% pull(lower)


sep_age_diet_data %>% filter(location_name == "China", rei_name == "Diet low in polyunsaturated fatty acids") %>% pull(lower)
diet_data %>% filter(location_name == "China", rei_name == "Diet low in polyunsaturated fatty acids") %>% pull(lower)
