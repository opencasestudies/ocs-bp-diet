...
diet_and_guidelines %>%
  ...
  filter(food == "red meat") %>%
  t.test(log10(mean_percent) ~ sex, data= ., 
         var.equal = TRUE)
...
means<-wide_diet %>% 
   filter(food== "fruits") %>% 
   summarise(female =mean(log10(mean_percent_Female)),
             male = mean(log10(mean_percent_Male)))
...
all_age_diet_and_guidelines %>%
  filter(food =="red meat") %>%
  ggplot(aes(sample = log10(mean_percent))) +
...
all_age_diet_and_guidelines%>%
  filter(food == "red meat") %>%  
  lm(log10(mean_percent) ~age_group_name, data= .) %>%
  summary()
...
So in our case: 
$$df = 15 -1$$
The difference here is that with the `lm()` test we also get information about how the individual age groups influence `mean_percent` consumption of `fruits`. Notice that if we look at all the age groups:
```{r}
all_age_diet_and_guidelines %>% 
...
all_age_diet_and_guidelines %>%
  filter(food == "red meat")%>%
  ggplot(aes(y = mean_percent , x= age_group_name, col = sex))+
  geom_boxplot()+
  geom_jitter(aes(x =sex, y =mean_percent), 
...
all_age_diet_and_guidelines %>%
  filter(food == "red meat")%>%
  filter(mean_percent >100) %>%
  ggplot(aes(y = mean_percent,
...
all_age_diet_and_guidelines %>%
  filter(food == "red meat")%>%
  filter(mean_percent <100) %>%
  ggplot(aes(y = mean_percent , x= age_group_name, fill = sex))+
  geom_point(position=position_jitterdodge(), 
             aes(col = sex), 
             alpha = 3/10)+
  geom_boxplot(outlier.shape=NA)+
...
all_age_diet_and_guidelines %>%
  filter(food == "red meat")%>%
  filter(mean_percent>100) %>%
  ggplot(aes(y = mean_percent , x= age_group_name))+
  geom_boxplot()+
  geom_jitter(aes(x =sex, y =mean_percent), 
                width = .2, size =2, shape =21)+
  theme(axis.text.x = element_text(angle = 70, hjust = 1))
...
all_age_diet_and_guidelines %>%
  filter(food == "red meat")%>%
  ggplot(aes(y = mean_percent , x= sex, fill = age_group_name))+
...
```{r}
 ...
 my_gg <-g +geom_point_interactive(aes(tooltip = location_name), size = 2) 
...
all_age_diet_and_guidelines %>%
filter(location_name == "United States") %>%
ggplot(aes(y = percent_over_under , x= food, fill = sex))+
  theme(axis.text.x = element_text(angle = 70, hjust = 1))+
...
all_age_diet_and_guidelines %>%
filter(location_name == "United States") %>%
ggplot(aes(y = percent_over_under , x= food, fill = age_group_name))+
  theme(axis.text.x = element_text(angle = 70, hjust = 1))+
  facet_wrap(~direction, scales = "free") +
  geom_boxplot()+
 geom_point(position=position_jitterdodge(), 
