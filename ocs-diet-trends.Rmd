...
## Motivation
An [article](https://www.thelancet.com/action/showPdf?pii=S0140-6736%2819%2930041-8){target="_blank"} recently published in the lancet journal evaluated global dietary trends and the relationship of dietary factors with mortality and fertility.
...
This figure from the supplement shows the ranking of the 15 dietary risk factors based on the estimated number of attributable deaths and illustrates how the top 3 risk factors are often issues for many different regions of the world.
...
Many dietary factors have well-established associations with health risk. The authors that generated this data set identified 15 dietary factors that have probably health risk based on literature search.
...
There are also important nuances to keep in mind regrading some of the dietary factors. For example calcium consumption was calculated based on consumption of dairy products, however calcium can be acquired from other sources including plant-based sources. However in these data, the influence of plant-based consumption of calcium was also not accounted for, nor was supplementation through vitamin sources. 
...
Here is another method using the package `summarytools`:
...
Here we can see that the `table` object now contains the text from the 3rd page as a **single large character string**. However the text is difficult to read because of the column structure in the PDF. Now let's try to grab just the text in the table.
...
Only the capitalized form of the word `"Diet"` appears to be within the table, and it is not present in the preceding text (although `"diet"` is). All the rows of interest of the table appear to start with the word `"Diet"`.
...
In this case we are also using the magrittr assignment pipe or double pipe that looks like this `%<>%` of the `magrittr` package. This allows us use the table data as input to the later steps but also reassign the output to the same data object name.
...
table %>%
  glimpse()
```
RStudio creates really helpful cheat sheets like this one which shows you all the major functions in the `stringr` package. You can download others [here](https://rstudio.com/resources/cheatsheets/){target="_blank"}.
...
It's important to realize that the first split will split the text before the first occurrence of `"Diet"` as the first value in the output. We could use the `first()` function of the `dplyr` package to look at this value. However, we will suppress the output as this is quite large.
...
Now we will try to split the strings for each row based on the presence of two spaces to create the columns of the table, as there appears to be larger than a space between the columns to create substrings. The substrings will be separated by quotes.
...
The second page of the `stringr` cheat sheet has more information about using "Special Characters" in `stringr`. For example `\\s` is interpreted as a space as the `\\` indicates that the `s` should be interpreted as a special character and not simply the letter s.  The `{2,}` indicates two or more spaces, while `{2}` would indicate exactly two spaces.
...
If we look closely, we can see that the sugar-sweetened beverage and the seafood category had only one space between the first and second columns. These are the columns about the dietary category and the one that describes in more detail what the consumption suggestion is about.
...
Since we originally split based on two or more spaces, we can just add a space in front of the word "Mean" for all the table strings and then try subsetting again. We can use the `str_which()` function of the `stringr` package to find the index of these particular cases.
...
Here we can use this function with `[]` to see just the strings that match these patterns within `table`:
```{r}
...
We could also just add a space in front of all the values of "Mean" in the table since the split was performed based on two or more spaces. Thus the other elements in `table` would also be split just as before despite the additional space.
...
This following will allow us to select the first or third substring from each element of the `table` object.
...
# The "[]" are necessary- will explain very soon
...
In order to create a tibble with our observed consumption rates with the suggested consumption rates, we will join our data using `dplyr`. In order to do so it is important that our different data sets have the same values. So let's first assess if that is the case.
...
Notice that it doesnt report rows being a different order becuase it first tells what rows have unique values or rows with a value that has a different number of frequency.
...
`setequal()` dones not provide clues about what is different but TRUE (no differences) or FALSE (indicating at least one difference)
...
`setdiff()` requires that columnames be the same so this will cause an error
...
Remember that the `food` data in our `guidelines` tibble is not necessarily in the same order as that of the consumption data tibbles. Thus this could be a problem if we decided to expand the `guidelines` rows (to repeat for the number of fruit observations etc.) and add them to our observed consumption tibbles. 
...
Thus lets calculate values of consumption that are relative to the suggested guidelines.
...
For those who are less familiar with the metric system where grams are equivalent to milliliters, it may be useful to realize how many fluid ounces the max amount of consumption per day (~248) grams actually is. 
...
Looks like overall, only `r 1520/4360*100`  of dietary factors for all tested populations were at optimal levels.
...
Looks as though on average the populations (both male and female separately) in Qatar, Rwanda, and Turkey consumed the optimal level of intake for the largest number of dietary factors (13 out of 30 (for the 15 dietary factors for males and females)).
...
Here is a way we can visualize this with the `ggplot2` package.
...
In order to make [inference](https://www.britannica.com/science/inference-statistics) about these comparisons, it is helpful to perform statistical tests. These tests can help us to determine the strength of the association between sex, age group and country identity with relative consumption of the dietary factors of interest. One way to look at the strength of association is to use a statistical method called regression.
...
It turns out that both the $t$-test and the ANOVA, are equivalent to specialized types of [regression](https://lindeloev.github.io/tests-as-linear/){target="_blank"}. We will use each of these tests to investigate patterns of consumption for dietary factors that contribute to health risk and we will look at how we can obtain equivalent results with regression.

### Regression

So what is regression? How can we use regression to compare our groups of interest and look at the relationship between group identity and consumption of dietary factors associated with health risk?

The statistical version of the term was coined in 1877 in this [article](http://galton.org/essays/1870-1879/galton-1877-typical-laws-heredity.pdf ){target="_blank"} about the relationship between hereditary traits and population averages. The author particularly focused on [height](https://zenodo.org/record/1449548#.Xlf_9hNKihc){target="_blank"} and kinship or relatedness. The word in general means to go back to a simpler mode. It was noticed that individuals with parents who had an extreme trait, such as exceptional height, tended to have a height more similar to the average of the population rather than the extreme height of their parents. For example if parents were very tall, their children were likely to be a bit shorter than their parents and therefore closer to the population average. Thus the children regressed towards the mean or in the author's words the offspring showed:
...
When we think about this from a statistical standpoint, regression allows us to estimate or **regress** relationships between variables to a "simple" model. We do this by **estimating the mean** of an outcome. This can be useful for **predicting future values** of the outcome based on the approximation of the real relationship between the variables within the model.
...
In this case, the slope or $\beta_{1}$ is 1 and the intercept is $\beta_{0}$ 10. We can see that if $X$ were 50, $Y$ would be approximately 60. This is very unusual in statistical analysis however, as often the relationship between variables is more complicated.
...
This concept has been extended to allow for comparisons of **different types of outcomes (the ys)**, to involve **various numbers of covariates (independent variables - the xs)**, and to allow for **different shapes of lines**. For a guide on how to perform regressions in R see [here](http://www.montefiore.ulg.ac.be/~kvansteen/GBIO0009-1/ac20092010/Class8/Using%20R%20for%20linear%20regression.pdf){target="_blank"}.
...
We have both continuous outcomes of raw consumption rates or percentages of consumption relative to the optimal guideline amounts as well as binary outcomes of "yes" or "no" about if a population achieved the optimal amount of consumption.

<b><u>Our covariates ($X_{i}$): </u></b>

We have data about age group, sex, and region information for all of our outcomes.

Again we are interested in comparing the sexes, the different age groups, and the different countries for their consumption of the different dietary factors. We can apply regression to determine if there is any influence of these group identities on the dietary consumption outcomes. 
...
We can evaluate if our data is [normally distributed](https://www.physiology.org/doi/full/10.1152/advan.00064.2017) by plotting the distribution and by creating [Q-Q plots](http://onlinestatbook.com/2/advanced_graphs/q-q_plots.html){target="_blank"}. 
...
N (normal) - The residuals are normally distributed. Note that the variables themselves do not need to be normally distributed.

E (equal variances) - The variance of the residuals for the groups is similar. This is called [homoscedasticity ](https://www.statisticssolutions.com/homoscedasticity/){target="_blank"}. In other words the residuals are of similar size along the line.
...
We can transform our data to make it more normally distributed. When data is highly right skewed, a log transformation is often helpful.
...
So now we can comparing the consumption of red meat by both genders using both a $t$-test and a linear regression. We can use the base `plot()` function to get information about our residuals. This helps us to assess if we are violating our linear regression assumptions. Typically we would first check the nontransformed data, because it is not required that the predictor variables have a normal distribution. However to allow for our comparison with the $t$-test, we will assess the log transformed `Relative_Percent` data.
...
Our second plot shows us that our residuals are a bit negatively or [left skewed](http://www.ucd.ie/ecomodel/Resources/QQplots_WebVersion.html), in this case just like the `Relative_Percent` data. However, linear regression is relatively robust to tailed distributions and again for the sake of comparison we will show the results here. It is possible that perhaps another term needs to be included in our model to improve this. We will check this in a bit.

We can see that our residuals are similar between males and females, as the first and third plot show similar values in the two lines. This is suggestive of [homoscedasticity ](https://www.statisticssolutions.com/homoscedasticity/){target="_blank"}. This like our variance test gives us information about the differences in variance between the two groups. Here is what the the plots would look like if the variance were not the same:
...
Notice how the $t$ value and the p-value are the same! (Well almost, the $t$ value is negative in the `t.test()` output because the female group is being used as reference group, while the male group is being used as the reference group in `lm()`). We can fix this using the `fct_inorder()` function of the `forcats` package which is all about factors. This function allows us to order the factor by what appears first. In this case "male" appears first, so now our output will match that of the `lm()` function.
...
First let's take a look at our plots for this new model:
...
Again lets see what our plots look like first:

```{r}

lmer_sex_loc<-lmerTest::lmer(
  log10(pull(filter(diet_and_guidelines, food == "red meat"),
                  Relative_Percent)) ~
  pull(filter(diet_and_guidelines, food == "red meat"),
                  sex)+
...
qqnorm(resid(lmer_sex_loc))
qqline(resid(lmer_sex_loc))
```

Notice that the plots look the same as with the fixed effect model. 
...
Notice here that the mean of the differences value is indeed the same as the coefficient and the $t$-value and the $p$-value match. The $\beta_{1}$ coefficient value (slope) matches the difference between the means value calculated in $t$-test. The intercept coefficient $\beta_{0}$ value of 1.56 (when sex is zero or female) also matches the mean value for the female group when calculated separately. 

### ANalysis Of VAriance (ANOVA) test

Now we are interested in the influence of age group on dietary consumption, because there are 15 age groups we can't assess the influence of age group on consumption using the $t$-test, as this test can only compare 2 groups. 
...
Thinking about our interest in wanting to know how consumption differs between age groups from the linear regression perspective, we could also describe our null hypothesis as:
...
We will also confirm again that our data is balanced for all groups, meaning we have the same or similar number of samples for each group.
...
Now we will use the `aov()` function of the `stats` package. We will be performing what is called a [one-way ANOVA](http://onlinestatbook.com/2/analysis_of_variance/one-way.html){target="_blank"} because we only have one dependent/response/outcome variable.
...
```
We can see that our `lm()` results are missing one of the age groups, the `25 to 29` age group. That is because this is the reference group and the coefficients indicate the slope or difference in `Relative_Percent` consumption rates for each listed age group compared to the reference group. 
...
We can also add links to Wikipedia pages for these countries using the `onclick` argument. See this [link](https://davidgohel.github.io/ggiraph/articles/offcran/using_ggiraph.html){target="_blank"} for more information on using the `ggirpah` package.
...
We would like to get a general sense of how consumption of these dietary factors differs around the world and we would like to know how the US compares to other countries. 
...
# this specifies how to separate the points for male and female
 ...
 scale_y_continuous(breaks=c(100,500, 1000, 2000), labels =c("100% thresh.",500, 1000, 2000) )+
...
Over <-Over + geom_point(data = diet_and_guidelines %>%
  ...
  scale_color_discrete(breaks="United States")+
    ...
    theme(plot.title = element_text(size = 40, 
        ...
        panel.background = element_rect(colour = "black", size=1.5))+
  expand_limits(y=-10)+
guides(colour = guide_legend(override.aes=list(shape = 15, size = 5, points = 0)))+
...
hmmm these look a bit funny because the outliers in the boxplots for the foods besides sugar-sweetened beverages are making the range much larger than the boxplot themselves. We can calculate the values for the boxplots our selves to deal with this:
...
OK, so females are a bit better about not over-consuming sodium in the United States relative to males.

Both groups are doing well with avoiding trans fatty acids.
...
Now let's take a look at the foods that were under-consumed:
...
Interesting, it looks like like both the younger adults and the elderly are generally doing worse than middle aged individuals in getting adequate intakes of these dietary factors.
...
It would be nice to scale the under-consumed foods with a max of 100%
