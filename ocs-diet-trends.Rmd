...
If we just look at differences only by sex in the data where all of the age groups are collapsed, there appears to be less differences in general, however males appear to potentially consume more sodium, fiber, calcium, red meat, and sugar-sweetened beverages than females. Females may consume more fruit.
...
In order to make [inference](https://www.britannica.com/science/inference-statistics) about these comparisons, it is helpful to perform statistical tests. First we are going to talk about a statistical method called regression.
...
Before we get started, let's remove the global values from our data and set them aside, as this is really a mean of all the country values.
...
Let's focus on the detary factors that appeared to potentially have a difference between genders based on our figure in our exploratory analysis.
...
How about the raw consumption data:
```{r}
diet_and_guidelines_long %>%
  filter(food == "red meat") %>%
  ggplot(aes(x = (mean))) +
  geom_histogram()
```
Also right skewed... so let's try taking the log of the mean value:
...
We can see that the slope of the linear model is equivalent to the difference bewteen the mean of the two groups, as the linear model line is parallel to the line that could be drawn between the two means.

Also, since we have female and male values each from the same countries, our data is what we called paired. It would be better if we modeled our data that way. First we will use the `pivot_wider()` function of  the `dplyr` package to get our data in a format that is easier to use for this purpose. To use this function we specify the values that we want to separate into more variables using the `values_from` argument and we use the `names_from` argument to specify how we want to separate these other variables. In this case we will make a male and female version of all the other variables specified.

```{r}

wide_diet <-pivot_wider(diet_and_guidelines, 
                        values_from= c(contains("percent"),
                                       mean, 
                                       upper, 
                                       lower, 
                                       opt_achieved), 
...
You can see that now our degrees of freedom are 194, which makes sense becuase now we have matched or paired values for the 195 different regions and so $df = n-1 = 195 -1 = 194$.

Again we get the same $t$-value and p-value with both tests.
...
Choose another dietary factor to compare between males and females.
### maybe we should have focused on these? sodium, low intake of whole grains, and low intake of fruits. 
...
It turns out that ANOVAs are also a specialized form of regression.
