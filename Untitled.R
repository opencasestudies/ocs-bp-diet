...
## Motivation
An [article](https://www.thelancet.com/action/showPdf?pii=S0140-6736%2819%2930041-8){target="_blank"} was recently published in the lancet journal that evaluates global dietary trends and the relationship of dietary factors with mortality and fertility.
...
y ~ x
```
Here our response/outcome variable is on the left of the `~` while our covariates or explanatory variables are on the right of the `~`.
...
It's also important that if there are more than one predictor variables, that these are not correlated.
...
So now we can comparing the consumption of red meat by both genders using both a $t$-test and a linear regression. We can use the base `plot()` function to get information about our residuals. This helps us to assess if we are violating our linear regression assumptions.
...
We can see that our residuals are similar between males and females, as the first and third plot show similar values in the two lines. This like our variance test gives us information about the differences in varaince between the two groups. Here is what the the plot would look like if the varaince were not the same:

```{r}
diet_and_guidelines %>%
  filter(food == "red meat") %>%
#mutate_if(sex = "Female", Relative_Percent +100) %>%
  #mutate_if(~is.numeric(.) && sex == "Female", log10)
  mutate(Relative_Percent =case_when(
  direction == "high" ~  (mean/upper_optimal)*100,
  direction == "low"  ~  (mean/lower_optimal)*100))
...
We would like to get a genearl sense of how consumption of these dietary factors differs around the world and we would like to know how the US compares to other countries. 

Before we do this let's change the labels of the foods so that they will fit more easily
...
#### Underconsumed foods
To choose the colors for our plot we can use show_col to preview color options from the viridis palette
```{r}
...
#create a larger black point for the US data
...
#create a smaller yellow point on top for the US data
...
#obesity_text<-tibble(Relative_Percent=c(350),food=c("sugar-sweetened beverages"),label=c("Optimal"))
Over<-diet_and_guidelines %>%
  ...
  geom_point(position=position_jitterdodge(), 
             ...
             alpha = 7/10,
             aes(fill = sex),
             size = 7)+ # this specifies how to separate the points
  geom_boxplot(outlier.shape = NA, 
               color = "black" , 
               lwd=2.5,
               aes(fill = sex))+
 ...
 scale_y_continuous(breaks=c(0,100,500, 1000, 2000), labels =c(0,"100% thresh.",500, 1000, 2000) )+
...
Over <-Over + geom_point(data = diet_and_guidelines %>%
    ...
    theme(plot.title = element_text(size = 40, hjust = 0.5, face = "bold"),
       ...
       axis.text.y = element_text(size = 35), 
        axis.title.y = element_text(size = 35), 
        axis.title.x = element_text(size =25),
       legend.text = element_text(size = 30, vjust = .01),
...
Over_age<-all_age_diet_and_guidelines %>%
  ...
  geom_hline(yintercept=100, 
             ...
             color = "red", 
             size =2) +
  facet_wrap_paginate(~food, scales = "free")+
  ...
  theme(axis.text.x = element_text(angle = 60, 
        ...
        legend.title = element_blank(),
        legend.position = "none")+
  labs(title=" Global consumption of foods\n associated with\n health risk when overconsumed",
       x ="", y = "Percent consumption relative \n to guidelines")+
  facet_wrap_paginate(~food, ncol = 2, scales = "free")+
...
hmmm these look a bit funny becuase the outliers in the boxplots for the foods besides sugar-sweetened beverages are making the range much larger than the boxplot themselves. We can calculate the values for the boxplots our selves to deal with this:
...
Under_age<-all_age_diet_and_guidelines %>%
  ...
  labs(title="\n Global consumption across age\n  of foods associated\n with health risk when underconsumed",
       x ="", y = "\nPercent consumption relative \n to guidelines")+
  scale_fill_viridis(discrete = TRUE)
...
```{r}
collapsed <-diet_and_guidelines %>% filter(food == "trans fatty acids")%>% select(mean, location_name)
notcollapsed <-all_age_diet_and_guidelines %>% filter(food == "trans fatty acids")%>% select(mean, location_name)
range(collapsed$mean)
range(notcollapsed$mean)
notcollapsed %>% arrange(desc(mean))
collapsed %>% arrange(desc(mean))
#hmmm the lowest value for malaysia is 106...
notcollapsed%>% filter(location_name == "Malaysia") %>% arrange(desc(-mean))
# but the collapsed value is much much lower...
collapsed%>% filter(location_name == "Malaysia")

#this is also fishy:
sep_age_diet_data %>% filter(food == "sodium", location_name== "Malaysia") %>%summarise(min = min(mean))
diet_and_guidelines%>%filter(food == "sodium", location_name== "Malaysia") %>% glimpse()
sep_age_diet_data %>% filter(food == "sodium", location_name== "Malaysia") %>%glimpse()
```



#### Underconsumed by Age
```{r, fig.height=13, fig.width=8, eval = FALSE}
Under_age<-all_age_diet_and_guidelines %>%
  filter(direction == "low") %>%
  filter(Relative_Percent <150)%>%
  ggplot(aes(y = Relative_Percent, 
             x = age_group_name, 
             fill = age_group_name)) +
  geom_boxplot(outlier.shape = NA, 
...
#{r, fig.height=38, fig.width = 32}
#cowplot::plot_grid(Over, Under, Over_age, Under_age, labels = c("A", "B","C", "D"), ncol = 2)

cowplot::plot_grid(Over, Over_age, Under, Under_age, ncol = 2, rel_widths = c(2.5, 1))
```
...
We learned that Regression is a powerful and flexible statistical tool that simplifies or estimates the relationships between variables using a mathematical model. We learned about the utility of the regression to compare groups, look for associations between variables, and to predict outcomes based on multiple predicitor or explanatory variables. We then compared this to other popular tests like the t-test and the ANOVA. We learned that these tests are actually equivalent to specialized types of regressions.

Our statistical analysis focused on evaluating differences in the consumption of red meat around the world between females and males and across different age groups. First we looked at the assumptions of [$t$-tests](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test){target="_blank"} and regressions, and determined that the relative percentage data of red meat consumption to the optimal guideline suggested amount was right skewed. We learned that we could transform the data by taking the log of these values to achieve more normally distrubted data. To compare males and females we used a t-test and learned that a t-test is a specialied form of a linear regression. To compare the 15 different age groups we used an anova and learned that the anova is also a specialized form of linear regression. We examined how we obtained the same results using either statistical test. This was also the case if we looked at the effect of gender and controlled for the paired country structure in the data by either including `location_name` in the model as another term or by using a mixed effects model to controll for this structure as a random effect but not specifically test for the influence of `location_name` on red meat consumption estimates. We learned that fixed effects are those that we wish to evaluate, while random effects are those that may infleunce the relationships of our variables of interest but that we do not wish to actively evaluate. Using these tests and models, we determined that males consume more red meat than women on average around the world. 

Our anova analysis of age determined that indeed there at least one age group consumed a significantly different amount of red meat compared to the other age groups, and this was still the case when we controlled `location_name`. However, we learned that we could that the anova does not provide information about which age groups are different. We learned how the regession however could provide some comparisons of different age groups relative to the reference age group. Furthermore, our data visualizations allowed us to determine that in general red met consumption appears to be higher in the younger age groups relative to the older age groups. 
...
We learned that we can use q-qplots to compare distributions to the normal distribution to evaluate the normality of our data.
...
3) [Levene's test](https://www.itl.nist.gov/div898/handbook/eda/section3/eda35a.htm){target="_blank"}, is more robust to violations of normality than the Bartlett's test, but not as robust as the Fligner- Killeen test.
    ...
    We see that the populations in many countries are overconsuming foods that are associated with health risk when overconsumed. In particular processed meat and sugar-sweetened beverages appear to be the most over consumed. Importantly botho of these appear to be consumed at higher quantities by males and younger adults. People in the US  appear to consume less sugar-sweetened beverages than other countries, however, people are still overconsuming. Processed meat however appears to be especially bad in the US. In terms of food that need to be consumed in adquate amounts to overcome health risk, nearly all countries for all factors are not reaching guideline levels. However, there are some countries consuming more than adequate amounts of legumes, vegitables, fruits and fiber. People in the US apear to eat more milk products and consume more omega-3 fatty acid and calcium rich foods than other countries. All countries including the US consume very low levels of polyunsaturated fatty acids. These [polyunsaturated fatty acids](https://en.wikipedia.org/wiki/Polyunsaturated_fat) are abundant in seads, nuts, avacodos, as well as fish. Likely the low level of consumption of nuts and seeds contributes to these low polyunsaturated fatty acid estimates. The supplementary table included in the article suggests that poor consumption of polyunsaturated fatty acids is associated with ischemic heart disease. The article takes this data further to evaluate the association of consumption levels of these foods with mortality.
    
    Analyses like the one in our case study are important for defining which groups could benefit the most from interventions, education, and policy changes when attempting to mitigate public health challenges. You can see in the [article](https://www.thelancet.com/action/showPdf?pii=S0140-6736%2819%2930041-8){target="_blank"} however that many additional considerations would be involved to perform a more thourough analysis to adequatly understand the data enough to recommend policy changes.
...
[RStudio cheatsheets](https://rstudio.com/resources/cheatsheets/){target="_blank”}
    [Interpunct](https://www.shorttutorials.com/mac-os-special-characters-shortcuts/middle-dot.html)
...
[residual](https://www.statisticshowto.datasciencecentral.com/residual/){target="_blank”}
    See [here](https://scientificallysound.org/2017/06/08/t-test-as-linear-models-r/){target="_blank"} for additional explanation about why that is the case. [Here](https://towardsdatascience.com/everything-is-just-a-regression-5a3bf22c459c){target="_blank"} and [here](https://lindeloev.github.io/tests-as-linear/){target="_blank"} are also great sources about how many commonly known statistical tests are specialized forms of regression.
    [normally distribution](https://www.physiology.org/doi/full/10.1152/advan.00064.2017)
    `lm()` see [here](https://feliperego.github.io/blog/2015/10/23/Interpreting-Model-Output-In-R){target="_blank”}.
...
[Satterthwaite formula](https://www.statisticshowto.datasciencecentral.com/satterthwaite-formula/){target="_blank”} 
    ...
    [$t$-tests](https://stattrek.com/statistics/dictionary.aspx?definition=two-sample%20t-test){target="_blank"} 
    Mood's test using [`mood.test()`](https://files.eric.ed.gov/fulltext/ED065559.pdf) (use if the data is not normally distributed)
[standard deviation](https://www.statsdirect.com/help/basic_descriptive_statistics/standard_deviation.htm){target="_blank"}

<u>Tests of Homogeneity of Variance<u/>
...
[polyunsaturated fatty acids](https://en.wikipedia.org/wiki/Polyunsaturated_fat)
...
[car](https://cran.r-project.org/web/packages/car/car.pdf)| to perform Levene's Test of Homogeneity of Variances
  [ggiraph](https://cran.r-project.org/web/packages/ggiraph/index.html)| to make plots interactive
  [ggforce](https://cran.r-project.org/web/packages/ggforce/ggforce.pdf)| to modify facets in plots
  [viridis](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html)| to plot in color pallette 
